...
constructNgramTables <- function(){
  ...
  en.blog <- sample(en.blog,round(length(en.blog)/15))
  en.news <- sample(en.blog,round(length(en.blog)/15))
  en.twitter <- sample(en.blog,round(length(en.blog)/15))
...
#  phrase <- removeWords(phrase,stopwords())
  ...
  # Sort the table, grabbing total probs and the words for the top 10
  pred.tbl <- head(pred.tbl[order(pred.tbl$prob.total, decreasing = TRUE),
                   c("Var1", "prob.total")],10)
